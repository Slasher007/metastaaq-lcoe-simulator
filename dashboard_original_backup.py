        ...
        with st.spinner(loading_message):
            try:
                ...
                for i, target_price in enumerate(target_prices):
                    ...
                    for year in df_plot.columns:
                        for month in df_plot.index:
                            if str(year) in extended_info and month in extended_info[str(year)]:
                                ...
                                spot_hours_data.loc[month, year] = 0
                                ppa_hours_data.loc[month, year] = 0
                    ...
                    if len(selected_years) > 1:
                        avg_spot = spot_hours_data.mean(axis=1)
                        avg_ppa = ppa_hours_data.mean(axis=1)
                        avg_total = avg_spot + avg_ppa
                        
                        x_pos = np.arange(len(df_plot.index))
                        width = 0.6
                        
                        ax1.bar(x_pos, avg_spot, width, label='Average Spot Hours', color='blue', alpha=0.8)
                        ax1.bar(x_pos, avg_ppa, width, bottom=avg_spot, label='Average PPA Hours', color='blue', alpha=0.5)
                    ...
                    if len(selected_years) == 1:
                        ax1.plot(x_pos, monthly_avg, color='red', linestyle='--', marker='o', label='Monthly Average')
                    ...
                    # Update title to indicate if average mode
                    if strategy_type == "Service Ratio-Based":
                        title = f'Operating Hours - {strategy_type} Strategy (PPA Threshold: {ppa_price}€/MWh)'
                    else:
                        title = f'Available Hours - {strategy_type} Strategy (Target: {target_price}€/MWh)'
                    if len(selected_years) > 1:
                    ...
                    ax1.set_ylabel('Operating Hours' if strategy_type == "Service Ratio-Based" else 'Available Hours')
